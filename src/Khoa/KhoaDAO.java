...
public class KhoaDAO implements IKhoaDAO {
    ...
    @Override
    public ArrayList<Khoa> findIDMaKH(String maKh) {
        ArrayList<Khoa> list = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        if (DBConnect.open()) {
            try {
                ps = DBConnect.cnn.prepareStatement("select *from tblKhoa where fldMaKhoa =?");
                ps.setString(1, maKh);
                rs = ps.executeQuery();
                list = new ArrayList<>();
                while (rs.next()) {
                    Khoa mk = new Khoa();
                    mk.setMakhoa(rs.getString(1));
                    mk.setTenkhoa(rs.getString(2));
                    mk.setSdt(rs.getString(3));
